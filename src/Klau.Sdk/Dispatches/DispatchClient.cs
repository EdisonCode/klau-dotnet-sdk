...
public sealed class DispatchClient
{
    ...
    internal DispatchClient(KlauHttpClient http) => _http = http;
    ...
    {
        return await _http.GetAsync<DispatchBoard>($"api/v1/dispatches/board?date={date}", ct);
    }
    ...
    {
        return await _http.PostAsync<OptimizationJob>("api/v1/dispatches/optimize", request, ct);
    }
    ...
    public async Task<OptimizationJob> GetOptimizationStatusAsync(
        ...
        CancellationToken ct = default)
    {
        return await _http.GetAsync<OptimizationJob>($"api/v1/dispatches/optimize/{jobId}", ct);
    }
    ...
    {
        await _http.PostAsync("api/v1/dispatches/publish", new { date }, ct);
    }
    ...
    {
        await _http.PostAsync($"api/v1/dispatches/{dispatchId}/reorder", request, ct);
    }
    ...
    {
        return await _http.PostAsync<WhatIfResult>("api/v1/dispatches/what-if", request, ct);
    }
}
